...
namespace CvPlot
{
        ...
        : _mouseAdapter(axes), _windowName(windowName)
    {
        ...
        cv::setMouseCallback(
            windowName, [](int event, int x, int y, int flags, void *userdata)
            {
    ...
    inline Window::Window(Window &&a)
        : _mouseAdapter(std::move(a._mouseAdapter)), _windowName(std::move(a._windowName)), _mat(std::move(a._mat))
    {
        setMouseCallback();
    }
    ...
    inline int Window::waitKey(int delay)
    {
    ...
    {
        cv::setMouseCallback(
            _windowName, [](int event, int x, int y, int flags, void *userdata)
            {
		Window& window = *static_cast<Window*>(userdata);
		window.updateSize();
		MouseEvent mouseEvent(window._mouseAdapter.getAxes(), window._mat.size(), event, x, y, flags);
